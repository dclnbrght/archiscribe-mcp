...
- [x] **5) MCP Typescript SDK integration & tool implementations**
   ...
   - [x] Define four MCP tools that expose ArchiMate model functionality:
       - [x] `SearchViews`: Input: { query?: string }, Output: { markdown: string }
       - [x] `GetViewDetails`: Input: { viewname: string }, Output: { markdown: string }
       - [x] `SearchElements`: Input: { query?: string, type?: string }, Output: { markdown: string }
       - [x] `GetElementDetails`: Input: { elementname: string }, Output: { markdown: string }
...
- Model shape (minimal):
  ...
  - Element: { id, name, type, documentation?, properties, inViews: [viewId], outgoingRelations: [relId], incomingRelations: [relId] }
...
- Unit: ModelLoader parses sample XML, returns known view names and elements
- Unit: Renderer produces markdown sections for view and element details
- Integration: Start server with sample model; call `/views?query=` and `/views/:id` and assert HTTP 200 and markdown content
- Integration: Start server with sample model; test element search by name and type, and element details retrieval
